...
(library (srfi171)
  ...
  (define (make-replacer map)
	(cond
	 ...
	 ((hashtable? map)
      (lambda (x)
		(hashtable-ref map x x)))
	 ((procedure? map) map)
	 (else
      (error 'treplace "Unsupported mapping in treplace" map))))
  ...
  (define tdelete-duplicates
	(case-lambda
      (() (tdelete-duplicates equal-hash equal?))
      ((hasher equality-pred?)
       (lambda (reducer)
		 (let ((already-seen (make-hashtable hasher equality-pred?)))
           (case-lambda
			 (() (reducer))
			 ((result) (reducer result))
			 ((result input)
              (if (hashtable-contains? already-seen input)
                  result
                  (begin
					(hashtable-set! already-seen input #t)
					(reducer result input))))))))))

  (define (vector-prefix->list vec n)
	(let loop ([acc '()]
			   [i 0])
	  (if (fx< i n)
		  (loop (cons (vector-ref vec i) acc)
				(fx1+ i))
		  (reverse! acc))))
  ...
  (define (tsegment n)
	(if (not (and (integer? n) (positive? n)))
		(error 'tsegment "argument to tsegment must be a positive integer")
		(lambda (reducer)
          (let ((i 0)
			...
			(case-lambda
              ...
              ((result)
               ...
               (let ((result
                      (if (zero? i)
                          result
                          (reducer result (vector-prefix->list collect i)))))
				 (set! i 0)
              ...
              ((result input)
               ...
               (if (< i n)
                   result
                   (let ((next-input (vector-prefix->list collect i)))
					 (set! i 0)
